...
export default {
  ...
  methods: {
    ...
    getDaysList(year, month, value) {
      ...
      value[2] =
        dayLength < this.dayList.length && this.pickerValue[2] > dayLength - 1
          ? dayLength - 1
          : this.pickerValue[2];
    ...
    show(defaultDate) {
      ...
      if (
        ...
        day = defaultDate.length > 0 ? defaultDate[2] : NOW_DATE.getDate();
      }
    ...
    _$emit(emitName) {
      ...
      let pickObj = {
        ...
        }-${this.dayList[value[2]].label}`,
        value: `${this.yearList[value[0]].value}-${
          this.monthList[value[1]].value
        }-${this.dayList[value[2]].value}`,
        index: value
      };
